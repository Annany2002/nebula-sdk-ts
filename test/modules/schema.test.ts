...
describe("SchemaModule", () => {
  ...
  it("define should throw BadRequestError on 400 (e.g., table exists, bad type)", async () => {
    ...
    await expect(client.schema.define(dbName, samplePayload)).rejects.toThrow(
      BadRequestError,
    );
    await expect(
      client.schema.define(dbName, samplePayload),
    ).rejects.toMatchObject({ errorData: errorResponse });
  });
  ...
  it("define should throw NotFoundError if database doesnt exist (404)", async () => {
    ...
    nock(mockBaseURL, { reqheaders: { Authorization: `Bearer ${token}` } })
      .post(
        `/api/v1/databases/${nonExistentDb}/schema`,
        JSON.stringify(samplePayload),
      )
      .reply(404, errorResponse);

    await expect(
      client.schema.define(nonExistentDb, samplePayload),
  ...
  it("define should throw validation error for invalid input", async () => {
    ...
    await expect(
      client.schema.define(dbName, { table_name: "t" } as any),
    ).rejects.toThrow(
      "Table name and at least one column definition are required.",
    );
    await expect(
      client.schema.define(dbName, { table_name: "t", columns: [] }),
    ).rejects.toThrow(
      "Table name and at least one column definition are required.",
    );
  });
  ...
  it("listTables should throw NotFoundError if database doesnt exist (404)", async () => {
    ...
    await expect(client.schema.listTables(nonExistentDb)).rejects.toThrow(
      NotFoundError,
    );
  });

  it("listTables should throw validation error if dbName is missing", async () => {
    await expect(client.schema.listTables("")).rejects.toThrow(
      "Database name is required.",
    );
  });
  ...
  it("deleteTable should send DELETE request for the table name", async () => {
    ...
    await expect(
      client.schema.deleteTable(dbName, tableName),
  ...
  it("deleteTable should throw NotFoundError on 404 (db or table not found)", async () => {
    ...
    await expect(client.schema.deleteTable(dbName, tableName)).rejects.toThrow(
      NotFoundError,
    );
  });

  it("deleteTable should throw validation error for missing names", async () => {
    await expect(client.schema.deleteTable("", "t1")).rejects.toThrow(
      "Database name is required.",
    );
    await expect(client.schema.deleteTable("db1", "")).rejects.toThrow(
