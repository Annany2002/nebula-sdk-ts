...
describe("RecordModule", () => {
  ...
  it("create should POST record data and return new record with ID", async () => {
    ...
    const result = await client.records.create(
      dbName,
      tableName,
      sampleRecordData,
    );
  ...
  it("create should throw BadRequestError on 400 (schema mismatch)", async () => {
    ...
    await expect(
      client.records.create(dbName, tableName, sampleRecordData),
  ...
  it("list should throw NotFoundError on 404 (db/table not found)", async () => {
    ...
    await expect(client.records.list(dbName, tableName)).rejects.toThrow(
      NotFoundError,
    );
  });
  ...
  it("get should throw NotFoundError on 404 (record not found)", async () => {
    ...
    await expect(
      client.records.get(dbName, tableName, nonExistentId),
    ).rejects.toThrow(NotFoundError);
  });
  ...
  it("get should throw validation error for invalid recordId", async () => {
    ...
    await expect(
      client.records.get(dbName, tableName, "abc" as any),
    ).rejects.toThrow("Record ID must be a positive integer.");
  });
  ...
  it("update should throw BadRequestError on 400 (schema mismatch)", async () => {
    ...
    await expect(
      client.records.update(dbName, tableName, recordId, updatePayload),
    ).rejects.toThrow(BadRequestError);
  });
  ...
  it("update should throw validation error for empty payload", async () => {
    await expect(
      client.records.update(dbName, tableName, recordId, {}),
  ...
  it("delete should send DELETE request for the record ID", async () => {
    ...
    await expect(
      client.records.delete(dbName, tableName, recordId),
